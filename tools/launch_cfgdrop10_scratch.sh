#!/usr/bin/env bash
# From-scratch CFG training: dropout=0.1 from step 0, 4 GPUs (same NUMA node),
# batch 2 per rank (global 8), lr 1.5e-5 (sqrt-scaled from 1e-5@global4),
# 150k steps (= 1.2M samples, same budget as 300k @ global 4).
set -euo pipefail
...
exec env \
    ...
    --run-name ray_posterior_cfgdrop10_scratch_20260903 \
    --steps 300000 \
    --batch-size 1 \
    --num-workers 2 \
