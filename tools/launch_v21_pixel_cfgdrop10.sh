#!/usr/bin/env bash
# V2.1 from-scratch CFG training: pixel LiDAR features, dropout=0.1,
...
if [[ "${RUN_V21_PIXEL_CFGDROP10:-0}" != "1" ]]; then
    cat <<'EOF'
V2.1 pixel CFG-drop10 launcher is ready but did not start training.
Set RUN_V21_PIXEL_CFGDROP10=1 to run it explicitly.
EOF
    exit 0
fi
...
exec env \
    ...
    --run-name pixel_lidar_v21_cfgdrop10 \
    --steps 300000 \
    --batch-size 1 \
    --num-workers 2 \
