#!/usr/bin/env bash
# Run on the KITTI server after Stage A and the four-GPU Stage B smoke pass.
set -euo pipefail
...
smoke_dir="$experiment_root/stage_b_smoke_scale1024"

# A second launcher must not start another job while this one is alive.
exec 9>"$experiment_root/pilot.lock"
flock -n 9
"$experiment_python" tools/summarize_geometry_history_run.py --run-dir "$smoke_dir"
"$experiment_python" - "$experiment_root" <<'PY'
import json
...
summary = json.loads((root / 'stage_b_smoke_scale1024/summary.json').read_text())
if summary['status'] != 'complete':
    raise RuntimeError('Stage B is incomplete')
for key, value in summary['smoke'].items():
    passed = value.get('pass', False) if isinstance(value, dict) else value
    if passed is not True:
        raise RuntimeError('Stage B gate failed: ' + key)
if (root / 'stage_c_pilot').exists():
    raise FileExistsError('Stage C output already exists; inspect it before resubmitting')
...
exec env CUDA_VISIBLE_DEVICES=4,5,6,7 OMP_NUM_THREADS=2 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=2 \
    ...
    --out-dir "$experiment_root/stage_c_pilot" \
    ...
    --resume "$smoke_dir/geometry_history_step_20.pt" \
