...
# Keep the current 200k run intact, then continue only from its verified final checkpoint.
CURRENT_PID="${1:?usage: $0 <current-train-pid>}"
MIN_STEP="${2:-200000}"
TARGET_STEP="${3:-1000000}"
...
RUN_NAME="raea_utonia_dino_full100_memmap_b2_resume_200k"
...
RESUME_STEP="$(${PYTHON} - "${CHECKPOINT}" <<'PY'
import sys
import torch

payload = torch.load(sys.argv[1], map_location="cpu")
print(int(payload.get("step", 0)))
PY
)"

if (( RESUME_STEP < MIN_STEP )); then
    echo "refusing continuation: checkpoint step ${RESUME_STEP} < required ${MIN_STEP}" >&2
    exit 1
fi
