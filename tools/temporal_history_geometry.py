"""Dense rotation history with measured LiDAR depth correspondences as anchors.

Unknown-depth cells use a far-scene (rotation-only) approximation, not an
identity copy or a claimed depth estimate. Measured cells rejected by LiDAR
geometry never fall back to this approximation. All correspondences are
current-query -> previous-history, for align_corners=False grid_sample.
"""
...
def build_lidar_history_geometry_from_arrays(
...
def _rotation_history_grid(
    cur_to_prev_velo: np.ndarray,
    geometry: RawKittiGeometry,
    grid: Tuple[int, int],
    image_size: Tuple[int, int],
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Project camera rays at infinity; translation deliberately has no effect."""
    h, w = grid
    if any(not isinstance(v, (int, np.integer)) or isinstance(v, bool) or v <= 0 for v in grid):
        raise ValueError("grid dimensions must be positive integers")
    if len(image_size) != 2 or any(not np.isfinite(v) or v <= 0 for v in image_size):
        raise ValueError("image size must be positive and finite")
    pose = np.asarray(cur_to_prev_velo, dtype=np.float64)
    if pose.shape != (4, 4) or not np.isfinite(pose).all():
        raise ValueError("relative pose must be finite 4x4")
    if not np.allclose(pose[3], [0, 0, 0, 1], atol=1e-6):
        raise ValueError("relative pose must be homogeneous")
    rotation = pose[:3, :3]
    if not np.allclose(rotation.T @ rotation, np.eye(3), atol=1e-5) or not np.isclose(np.linalg.det(rotation), 1., atol=1e-5):
        raise ValueError("relative pose must have a proper rotation")
    k = np.asarray(geometry.p_rect_02[:, :3], dtype=np.float64)
    camera_rotation = (geometry.r_rect_00_ext @ geometry.t_cam_velo)[:3, :3]
    if not np.isfinite(k).all() or not np.isfinite(camera_rotation).all():
        raise ValueError("camera calibration must be finite")
    try:
        camera_relative = camera_rotation @ rotation @ np.linalg.inv(camera_rotation)
    ...
    # Inverse of image_to_feature_xy and of project_velo_to_image resizing.
    u = ((xx + .5) * img_w / w - .5) * src_w / img_w
    v = ((yy + .5) * img_h / h - .5) * src_h / img_h
    rays = inv_k @ np.stack([u, v, np.ones_like(u)], axis=0).reshape(3, -1)
    rotated = camera_relative @ rays
    pixels = k @ rotated
    forward = rotated[2] > 1e-9
    divisor = np.where(np.abs(pixels[2]) > 1e-9, pixels[2], 1.)
    previous_uv = (pixels[:2] / divisor).T
    previous_uv[:, 0] *= img_w / src_w
    previous_uv[:, 1] *= img_h / src_h
    previous_xy = image_to_feature_xy(previous_uv, image_size, grid).reshape(h, w, 2)
    # Restrict to feature-center support: no padding-zero bleed at image edges.
    eps = 1e-5
    valid = (forward.reshape(h, w) & np.isfinite(previous_xy).all(axis=-1)
             & (previous_xy[..., 0] >= -eps) & (previous_xy[..., 0] <= w - 1 + eps)
             & (previous_xy[..., 1] >= -eps) & (previous_xy[..., 1] <= h - 1 + eps))
    previous_xy[..., 0] = np.clip(previous_xy[..., 0], 0, w - 1)
    previous_xy[..., 1] = np.clip(previous_xy[..., 1], 0, h - 1)
    previous_xy[~valid] = 0
    return align_corners_false_normalize(previous_xy, grid), previous_xy, valid
...
def build_history_geometry_from_arrays(
    ...
    """Default B geometry: rotation fallback for unknown depth, LiDAR first.

    Missing depth is not a rejection. However a measured current cell whose
    LiDAR correspondence is unsupported/conflicting/out of view stays invalid.
    Rotation fallback does not model translation parallax, object motion, or
    unseen occlusions. Provenance masks expose that uncertainty to the caller.
    """
    image_size = image_size or geometry.image_size
    rotation_grid, rotation_px, rotation_in = _rotation_history_grid(
        cur_to_prev_velo, geometry, grid, image_size)
    if not np.isfinite(depth_tol_m) or depth_tol_m < 0:
        raise ValueError("depth tolerance must be finite and nonnegative")
    result = build_lidar_history_geometry_from_arrays(
        prev_points_velo, cur_points_velo, cur_to_prev_velo, geometry,
        grid=grid, image_size=image_size, depth_tol_m=depth_tol_m)
    lidar_valid = result['history_valid'].copy()
    rotation_valid = rotation_in & ~result['current_covered']
    rejected = result['current_covered'] & ~lidar_valid
    from tools.temporal_object_geometry import recover_moving_object_history
    recovered = recover_moving_object_history(
        prev_points_velo, cur_points_velo, cur_to_prev_velo, geometry,
        lidar_valid, result['current_covered'], grid, image_size, depth_tol_m)
    object_valid = recovered['object_valid'] & ~lidar_valid
    result['history_grid'][object_valid] = recovered['object_grid'][object_valid]
    result['history_grid_px'][object_valid] = recovered['object_grid_px'][object_valid]
    result['history_grid'][rotation_valid] = rotation_grid[rotation_valid]
    result['history_grid_px'][rotation_valid] = rotation_px[rotation_valid]
    yy, xx = np.indices(grid, dtype=np.float32)
    result['current_grid_px'] = np.stack([xx, yy], axis=-1)
    result['history_valid'] = lidar_valid | object_valid | rotation_valid
    result['lidar_valid'] = lidar_valid
    result['object_valid'] = object_valid
    result['rotation_valid'] = rotation_valid
    result['known_rejected'] = rejected & ~object_valid
    source = np.zeros(grid, dtype=np.uint8)
    source[rotation_valid] = 1
    source[lidar_valid] = 2
    source[object_valid] = 3
    result['history_source'] = source
    metrics = _metrics(result['history_valid'], result['current_covered'],
                       result['ground_proxy'], result['non_ground_proxy'],
                       result['current_grid_px'], result['history_grid_px'], grid)
    obj = recovered['diagnostics']
    metrics.update({
        'coverage_of_current_lidar': float(lidar_valid.sum()) / max(float(result['current_covered'].sum()), 1.),
        'coverage_lidar': float(lidar_valid.mean()),
        'coverage_object': float(object_valid.mean()),
        'coverage_rotation': float(rotation_valid.mean()),
        'coverage_known_rejected': float(result['known_rejected'].mean()),
        'coverage_rotation_out_of_view': float((~rotation_in & ~result['current_covered']).mean()),
        'lidar_history_cell_count': int(lidar_valid.sum()),
        'object_history_cell_count': int(object_valid.sum()),
        'rotation_history_cell_count': int(rotation_valid.sum()),
        'object_current_clusters': obj.get('current_clusters', 0),
        'object_previous_clusters': obj.get('previous_clusters', 0),
        'object_associated': obj.get('associated', 0),
        'object_moving_accepted': obj.get('moving_accepted', 0),
        'object_stationary_skipped': obj.get('stationary_skipped', 0),
        'object_rejected_align': obj.get('rejected_align', 0),
        'object_unrecovered_rejected_cells': obj.get('unrecovered_rejected_cells', 0),
        'history_mode': 'lidar_static_object_motion_rotation_background',
        'note': 'Object cells are ego-rejected measured cells recovered with a rigid object transform. Rotation is far-scene only.',
    })
    result['metrics'] = metrics
    result['object_diagnostics'] = obj
    return result
