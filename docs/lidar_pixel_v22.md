...
optimizer/AMP/step. Explicit `--init-ckpt /path/to/v21/step_020000.pt` loads V2.1
model weights, allows only the new pixel depth head's missing keys, leaves that
head newly initialized, and starts a new optimizer and local step counter.
Initialization provenance is saved in the checkpoint metadata. Never interpret
V2.2 local step 1 as one total step of training when initialized from V2.1.
...
Full training remains paused after the bounded functional checks. These checks
establish implementation behavior, not that V2.2 improves image quality.
...
- 58 targeted tests passed, covering adjacent foreground/background hit targets,
  ...
  native configuration, strict checkpoint handling and initialization provenance.
- Four RTX 3090 GPUs completed two AMP optimizer steps initialized from V2.1
  step 20000, with zero skipped updates. Actual depth logs report 128x512,
...
remain unverified; the 300k training job has not been restarted.
