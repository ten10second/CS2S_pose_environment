"""Temporal history attention: geometry-gated appearance memory (v3 design).
...
Components:
  HistoryLatentEncoder — maps the previous frame's final latent
      ẑ[t-1] (b,4,16,64) to history tokens (b, T, C_h) on a fixed grid.
  GeometryHistoryAttention — one per fusion block. The QUERY receives the
      current conditions' fused summary (sat+LiDAR deltas, detached) plus the
      query's own features; K/V come from the history tokens, biased toward the
      LiDAR/pose correspondence cell. Invalid cells and has_history=False are
      exact zero. has_history=False still routes a learned null token through
      the K/V weights so every parameter stays in the DDP graph, and the output
